...
class SeleniumLibrary(DynamicCore):
    ...
    SeleniumLibrary uses the RobotFrameworkSelenium WebDriver modules internally to
    control a web browser. See http://seleniumhq.org for more information
    about RobotFrameworkSelenium in general and SeleniumLibrary README.rst
    ...
    *NOTE:*
    ...
    RobotFrameworkSelenium's WebElement objects. This requires first getting a WebElement,
    ...
    will create a new RobotFrameworkSelenium WebDriver instance by using the
    [https://www.seleniumhq.org/docs/03_webdriver.jsp|RobotFrameworkSelenium WebDriver]
    API. In SeleniumLibrary terms, a new browser is created. It is
    possible to start multiple independent browsers (RobotFrameworkSelenium Webdriver
    ...
    users, two browsers (RobotFrameworkSelenium WebDriver instances) must be created.
    ...
    using the `Set RobotFrameworkSelenium Timeout` keyword or with the ``timeout`` argument
    when `importing` the library. See `time format` below for supported
    timeout syntax.

    == Implicit wait ==

    Implicit wait specifies the maximum time how long RobotFrameworkSelenium waits when
    searching for elements. It can be set by using the `Set RobotFrameworkSelenium Implicit
    ...
    RobotFrameworkSelenium documentation] for more information about this functionality.

    See `time format` below for supported syntax.

    == RobotFrameworkSelenium speed ==

    RobotFrameworkSelenium execution speed can be slowed down globally by using `Set
    RobotFrameworkSelenium speed` keyword. This functionality is designed to be used for
    ...
    See the RobotFrameworkSelenium and SeleniumLibrary
    ...
    RobotFrameworkSelenium tool is not thread-safe] within one browser/driver instance.
    Because of the limitation in the RobotFrameworkSelenium side, the keywords or the
    ...
    def __init__(self, timeout=5.0, implicit_wait=0.0,
        ...
        - ``event_firing_webdriver``:
          Class for wrapping RobotFrameworkSelenium with
          [https://seleniumhq.github.io/selenium/docs/api/py/webdriver_support/selenium.webdriver.support.event_firing_webdriver.html#module-selenium.webdriver.support.event_firing_webdriver|EventFiringWebDriver]
        """
    ...
    def register_driver(self, driver, alias):
        ...
        :param driver: Instance of the RobotFrameworkSelenium `WebDriver`.
    ...
    def _parse_listener(self, event_firing_webdriver):
        ...
        if not isclass(listener):
            message = "Importing test RobotFrameworkSelenium lister class '%s' failed." % listener_module.module
            raise DataError(message)
        return listener
