...
class ElementKeywords(LibraryComponent):
    ...
    def element_should_be_focused(self, locator):
        ...
        # RobotFrameworkSelenium 3.6.0 with Firefox return dict wich contains the selenium WebElement
...
newDiv.parentNode.style.overflow = 'hidden';
    ...
    def click_element(self, locator, modifier=False, action_chain=False):
        ...
        [https://seleniumhq.github.io/selenium/docs/api/py/webdriver/selenium.webdriver.common.keys.html#selenium.webdriver.common.keys.Keys|RobotFrameworkSelenium Keys]
        when clicking the element. The `+` can be used as a separator
        for different RobotFrameworkSelenium Keys. The `CTRL` is internally translated to
        ...
        . If ``modifier`` does not match to RobotFrameworkSelenium Keys, keyword fails.
        ...
        | Click Element | id:button | action_chain=True | # Clicks the button using an RobotFrameworkSelenium  ActionChains        |
    ...
    def click_element_at_coordinates(self, locator, xoffset, yoffset):
        ...
        element = self.find_element(locator)
        action = ActionChains(self.driver)
        # Try/except can be removed when minimum required RobotFrameworkSelenium is 4.0 or greater.
        try:
            action.move_to_element(element)
        except AttributeError:
            self.debug('Workaround for RobotFrameworkSelenium 3 bug.')
...
return !element.dispatchEvent(evt);
    ...
    def press_keys(self, locator=None, *keys):
        ...
        [https://seleniumhq.github.io/selenium/docs/api/py/webdriver/selenium.webdriver.common.keys.html|RobotFrameworkSelenium Keys]
        and strings or a single RobotFrameworkSelenium Key. If RobotFrameworkSelenium Key is combined
        with strings, RobotFrameworkSelenium key and strings must be separated by the
        `+` character, like in `CONTROL+c`. RobotFrameworkSelenium Keys
        are space and case sensitive and RobotFrameworkSelenium Keys are not parsed
        inside of the string. Example AALTO, would send string `AALTO`
        and `ALT` not parsed inside of the string. But `A+ALT+O` would
        found RobotFrameworkSelenium ALT key from the ``keys`` argument. It also possible
        to press many RobotFrameworkSelenium Keys down at the same time, example
        'ALT+ARROW_DOWN`.

        If RobotFrameworkSelenium Keys are detected in the ``keys`` argument, keyword
        will press the RobotFrameworkSelenium Key down, send the strings and
         then release the RobotFrameworkSelenium Key. If keyword needs to send a RobotFrameworkSelenium
        Key as a string, then each character must be separated with
        `+` character, example `E+N+D`.

        `CTRL` is alias for
        [https://seleniumhq.github.io/selenium/docs/api/py/webdriver/selenium.webdriver.common.keys.html#selenium.webdriver.common.keys.Keys.CONTROL|RobotFrameworkSelenium CONTROL]
        and ESC is alias for
        [https://seleniumhq.github.io/selenium/docs/api/py/webdriver/selenium.webdriver.common.keys.html#selenium.webdriver.common.keys.Keys.ESCAPE|RobotFrameworkSelenium ESCAPE]
    ...
    def parse_modifier(self, modifier):
        ...
        for item in modifiers:
            ...
            if hasattr(Keys, item):
                ...
                raise ValueError("'%s' modifier does not match to RobotFrameworkSelenium Keys"
                                 % item)
        return keys
